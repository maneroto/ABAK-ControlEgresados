(function ($) {
  ...
  $.fn.material_select = function (callback) {
    $(this).each(function(){
      ...
      var filterQuery = [],
          onKeyDown = function(e){
            ...
            // ARROW UP - move to antes not disabled option
            if (e.which == 38) {
    ...
    function toggleEntryFromArray(datosArray, entryIndex, select) {
      var index = datosArray.indexOf(entryIndex),
          notAdded = index === -1;

      if (notAdded) {
        datosArray.push(entryIndex);
      } else {
      ...
      setValueToInput(datosArray, select);

      return notAdded;
    }

    function setValueToInput(datosArray, select) {
      var value = '';

      for (var i = 0, count = datosArray.length; i < count; i++) {
        var text = select.find('option').eq(datosArray[i]).text();
