...
$(document).ready(function(){
    ...
    $("#lada_fijo").on('input',function(){
        var lada_fijo = $('#lada_fijo').val();
        if(lada_fijo == "")
            $("#span_lada_fijo").text("Campo obligatorio");
        else if(!($.isNumeric(lada_fijo)))
             $("#span_lada_fijo").text("Sólo se admiten números");
        else if(lada_fijo.length < 2)
             $("#span_lada_fijo").text("Deben ser al menos 2 dígitos");
        else
            $("#span_lada_fijo").text("");
    });
    ...
    $('#registrarEncuesta').submit(function(e) {
        ...
        $.ajax({
            type: "POST",
            url: '../php/alumnos_php/registrarEncuesta',//antes era:   ../../php/registrarEncuesta.php
            data: $(this).serialize(),
            success: function(data){
                alert(data);
                setTimeout(window.location = 'index.php?logout=true', 5000);
